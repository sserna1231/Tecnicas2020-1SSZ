...
typedef struct Statistics_execution{
    ...
    char* fecha_fin;
    float exec_time;
    int operaciones[3]; //Comparaciones, reposiciones, iteraciones
} stat;
...
void imprimirEstadisticas(stat *estadisticas_prueba, int Test_size, char* Algoritmo_name){
    ...
    if(output){
        ...
        fprintf(output, "Cantidad de iteraciones: %d\n", estadisticas_prueba->operaciones[2]);
    ...
    if(array){
        ...
        for(int i = 1; i <= 2; i++){
            ...
            if(data == Test_size){
                ...
                imprimirEstadisticas(&estadisticas, Test_size, "BubbleSort Fixed");
            } else{
